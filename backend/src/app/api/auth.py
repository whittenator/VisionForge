...
def login(req: LoginRequest) -> dict:
    ...
    # Fetch the full User object to build the response via get_db context.
    # authenticate() already verified credentials; we only need the ORM object
    # for the unified helper. Re-use the auth_user TypedDict for simplicity.
    return {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "token_type": "bearer",
...
def signup(req: SignupRequest, db: Session = Depends(get_db)) -> dict:
    ...
    try:
        ...
        # Idempotent: if the password matches an existing account, return tokens.
        result = auth_service.authenticate(str(req.email), req.password)
        if not result:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    result = auth_service.authenticate(user.email, req.password)
    if result is None:
        raise HTTPException(
